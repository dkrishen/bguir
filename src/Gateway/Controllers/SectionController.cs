...
public class SectionController : ControllerBase
{
    ...
    [HttpPost("import/")]  
    public async Task<IActionResult> Import([FromBody] string[] data)
    {
        await _sectionLogic.ImportDataAsync(data).ConfigureAwait(false);
        return Ok();
    }
    ...
    {
        var result = await _sectionLogic.ExportDataAsync().ConfigureAwait(false);
        return Ok(result);
    }
}